...
$tipo_pago = $this->get_meta($post->ID, self::META_TIPO_PAGO);
$fecha = $this->get_meta($post->ID, self::META_FECHA_PAGO);
$total = $this->get_meta($post->ID, self::META_TOTAL);
...
?>
<form method="post">
    <?php wp_nonce_field('save_registro_pago', 'registro_pago_nonce'); ?>
    <table class="form-table">
        ...
        <tr>
            <th><?php echo esc_html(__('Producto Solicitado:', self::PLUGIN_SLUG)); ?></th>
            <!-- {title post} - small link to view post -->
            <td><a href="<?php //echo get_permalink($this->get_meta($post->ID, self::META_POST_ID)); ?>" target="_blank"><?php //echo esc_html(get_the_title($this->get_meta($post->ID, self::META_POST_ID))); ?></a></td>
        </tr>
        <tr>
